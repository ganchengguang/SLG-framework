    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            custom_tokenizer = T5Tokenizer.from_pretrained('best_2_1csv_bs88_epoch5_1e4_512_256')
            # custom_tokenizer = AutoTokenizer.from_pretrained('bart-large-japanese')
            if cur_len == 1:
                custom_idx = custom_tokenizer.encode('<', add_special_tokens=False, return_tensors='pt').to('cuda:0')  #bart-base-japanese
                # custom_idx = custom_tokenizer.encode('〈', add_special_tokens=False, return_tensors='pt').to('cuda:0') #bart-large-kyoto-japanese
                custom_idx = custom_idx[0][1] #T5-base-japanese
                # print(custom_idx)
                next_tokens = torch.full((next_tokens.shape[0],), fill_value=custom_idx.item()).to('cuda:0')
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            custom_tokenizer = T5Tokenizer.from_pretrained('T5_japanese_base')
            if cur_len == 1:
                custom_idx = custom_tokenizer.encode('<', add_special_tokens=False, return_tensors='pt').to('cuda:0') #bart-base-japanese
                custom_idx = custom_idx[0][1]

                beam_next_tokens = torch.full((beam_next_tokens.shape[0],), fill_value=custom_idx.item()).to('cuda:0')
                # custom_tokenizer.convert_ids_to_tokens(custom_idx) 
